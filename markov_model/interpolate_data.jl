...
##
# load in files 
hfile = h5open("markov_model_even_time_nstate_400.h5")
...
hfile = h5open("interpolated_fields_n400_coarse.h5", "w")
for (i, state) in ProgressBar(enumerate(markov_states))
    for s in 1:5
        global oldf = view(state, :, :, s)
        global newf = view(interpolated_markov_state, :, :, :, s)
        interpolate_field!(newf, oldf, d_elist, d_ξlist, r, ω, Nq⃗, arch=CPU())
    end
    hfile["interpolated markov state $i"] = newf
    for i in 1:size(state)[1], j in 1:size(state)[2]
        sphere_state[i, j, :] .= mean_variables(state[i, j, :], x⃗[i, j], Φ[i, j])
    end
    hfile["sphere markov state $i"] = sphere_state
    for s in 1:5
        global oldf = view(sphere_state, :, :, s)
        global newf = view(interpolated_markov_state, :, :, :, s)
        interpolate_field!(newf, oldf, d_elist, d_ξlist, r, ω, Nq⃗, arch=CPU())
    end
    hfile["interpolated sphere markov state $i"] = newf
end
hfile["geopotential"] = Φ
close(hfile)