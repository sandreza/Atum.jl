...
struct DryTotalEnergyLaw{FT, D, S, C} <: Atum.AbstractBalanceLaw{FT, D, S, C}
function DryTotalEnergyLaw{FT, D}(; γ = 7 // 5,
                                    grav = 981 // 100,
...
function varsindices(law::DryTotalEnergyLaw)
...
function auxindices(law::DryTotalEnergyLaw)
    S = Atum.numberofstates(law)
    n_u⃗ = S - 2
    ix_x = 1
    ix_y = 2
...
# These are incorrect for a sphere: 
function Atum.auxiliary(law::DryTotalEnergyLaw, x⃗, state)
    ix_ρ, ix_ρu⃗, ix_ρe = varsindices(law)
...
function coordinates(law::DryTotalEnergyLaw, aux)
    aux[1:3]
end

function geopotential(law::DryTotalEnergyLaw, aux)
    aux[end]
end

function pressure(law::DryTotalEnergyLaw, ρ, ρu⃗, ρe, Φ)
    γ = constants(law).γ
    return (γ - 1) * (ρe - ρu⃗' * ρu⃗ / 2ρ - ρ * Φ)
end

function pressure(law::DryTotalEnergyLaw, state, aux)
...
function linearized_pressure(law::DryTotalEnergyLaw, state, aux)
...
function reference_pressure(law::DryTotalEnergyLaw, aux)
    x, y, z, ρᵣ, ρuᵣ, ρeᵣ, Φ = unpackaux(law, aux)
    γ = constants(law).γ
    pressure(law, ρᵣ, ρuᵣ, ρeᵣ, Φ)
end 

function soundspeed(law::DryTotalEnergyLaw, ρ, p)
    γ = constants(law).γ
    sqrt(γ * p / ρ)
end

function reference_soundspeed(law::DryTotalEnergyLaw, aux)
    x, y, z, ρᵣ, ρuᵣ, ρeᵣ, Φ = unpackaux(law, aux)
    γ = constants(law).γ
...
function modified_surfaceflux!(::Atum.RoeFlux, law::DryTotalEnergyLaw{FT,D,S,C}, flux, n⃗, q⁻, aux⁻, q⁺, aux⁺) where {FT,D,S,C}
    Φ = geopotential(law, aux⁻) # since continuous don't need + state
...
function Atum.surfaceflux(rf::Atum.RefanovFlux, law::DryTotalEnergyLaw, n⃗, q⁻, aux⁻, q⁺, aux⁺)
...
function Atum.surfaceflux(rf::Atum.LinearizedRefanovFlux, law::DryTotalEnergyLaw, n⃗, q⁻, aux⁻, q⁺, aux⁺)
...
function modified_volumeflux!(::Atum.KennedyGruberFlux, law::DryTotalEnergyLaw{FT,D,S,C},
...
##
function modified_volumeflux!(::Atum.LinearizedKennedyGruberFlux,
    law::DryTotalEnergyLaw{FT,D,S,C}, flux, state_1, aux_1, state_2, aux_2) where {FT,D,S,C}
